...
function handleSearch(query: string) {
  ...
  const typeStats = lastResults.reduce(
    (acc, item) => {
...
function handleExport(limitStr?: string) {
  if (lastResults.length === 0) {
    console.log(
      "❌ Aucun résultat à exporter. Effectuez d'abord une recherche."
    );
    return;
  }
...
function handleStats() {
  const typeStats = codeElements.reduce(
    (acc, item) => {
      acc[item.type] = (acc[item.type] || 0) + 1;
      return acc;
    },
  ...
  const fileStats = codeElements.reduce(
    (acc, item) => {
  ...
  Object.entries(typeStats)
    ...
    .forEach(([type, count]) => {
      const icon = getIcon(type);
      console.log(
        `  ${icon} ${type.padEnd(12)} ${count.toString().padStart(4)}`
      );
    });
