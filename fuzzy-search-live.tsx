#!/usr/bin/env bun
import { fuzzySearch, codeElements, type CodeElement, fuzzySearchAI, type SearchResult } from './parse-project';
...
function openInCursor(filePath: string, lineNumber: number) {
    ...
    exec(command, (error) => {
        if (error) {
            console.error(`Erreur lors de l'ouverture: ${error.message}, command: ${command}`);
        }
    });
}

// Fonction pour lire le contexte du code
function getCodeContext(filePath: string, lineNumber: number): { line: string; number: number }[] {
    try {
        const content = readFileSync(filePath, 'utf-8').split('\n');
        const start = Math.max(0, lineNumber - 5);
        const end = Math.min(content.length, lineNumber + 4);
    ...
    return (
        <Box flexDirection="column">
            ...
            <Box marginTop={1} flexDirection="column">
                {isLoading ? (
                    <Box flexDirection="row">
                        <Text>🔄 </Text>
                        <Text color="yellow">
                            {searchMode === 'ai' ? 'Recherche IA en cours...' : 'Recherche en cours...'}
                        </Text>
                    </Box>
                    ...
                    <SelectInput<SearchResult>
                        ...
                        itemComponent={(({ label, value, isSelected }: { label: string, value: CodeElement, isSelected: boolean }) => (
                            <Box flexDirection="column">
                                <Text>{label}</Text>
                                <Text>📁 {value.filePath}:{value.lineNumber}</Text>
                                {value.description && (
                                    <Text>📝 {value.description}</Text>
                                )}
                                {isSelected && (
                                    <Box marginTop={1} borderStyle="round" borderColor="blue" padding={1} flexDirection="column">
                                        {getCodeContext(value.filePath, value.lineNumber).map((context, index) => (
                                            <Box key={index} flexDirection="row">
                                                <Text color="gray">{context.number.toString().padStart(3, ' ')}: </Text>
                                                <Text color={index === 4 ? 'green' : 'gray'}>
                                                    {context.line}
                                                </Text>
                                            </Box>
                                        ))}
            ...
            {results.length > MAX_RESULTS && (
                <Box marginTop={1}>
                    <Text>... et {results.length - MAX_RESULTS} autres résultats</Text>
                </Box>
            )}

            <Box marginTop={2}>
                <Text>Utilisez ↑/↓ pour naviguer, Entrée pour ouvrir, Tab pour changer de mode</Text>
            </Box>
